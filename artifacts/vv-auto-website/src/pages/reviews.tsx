...
function GoogleBadge({ live }: { live: boolean }) {
  return (
    ...
    >
      ...
      <span className={`text-xs font-semibold transition-colors ${live ? "text-gray-500 group-hover:text-[#4285F4]" : "text-gray-300"}`}>
        Google Review
      </span>
      {live && (
        <ExternalLink size={10} className="text-gray-400 group-hover:text-[#4285F4] transition-colors" />
      )}
    </a>
  );
}
