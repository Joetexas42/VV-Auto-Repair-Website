 ...
 *
 * Verifies that every hardcoded Garland "Get Directions" href in the codebase:
 *   1. Uses the coordinate-based URL format (@lat,lng) — not a plain ?q= text query
 *   2. Contains the correct coordinates for 3730 Marquis Dr, Garland TX
 *   3. Contains the correct Google place feature-ID (11pzygbgln)
 *
...
for (const source of SOURCES) {
  ...
  const garlandUrls = (content.match(urlRegex) ?? []).filter((u) =>
    u.includes(EXPECTED_COORDS) || u.includes(EXPECTED_FEATURE_ID)
  );

  assert(
    garlandUrls.length >= source.expectedOccurrences,
    `At least ${source.expectedOccurrences} Garland maps URL(s) found (got ${garlandUrls.length})`
  );
